...
def engine_run_detail(db: Session, engine_run_id: str) -> PersistedEngineRunDetailRead | None:
    ...
    return PersistedEngineRunDetailRead(
        **summary.model_dump(),
        ...
        constraint_report=(engine_run.diagnostic_snapshot_json or {}).get("constraint_report", {}),
        generated_plan=engine_run.plan_snapshot_json or {},
...
def serialize_engine_run(db: Session, engine_run: EngineRun) -> PersistedEngineRunRead:
    ...
    return PersistedEngineRunRead(
        ...
        assessment_type=assessment.assessment_type if assessment is not None else report_json.get("assessment_type", "full_diagnostic"),
        assessment_type_label=report_json.get(
            "assessment_type_label",
            (assessment.assessment_type.replace("_", " ").title() if assessment is not None else "Full Diagnostic"),
        ),
