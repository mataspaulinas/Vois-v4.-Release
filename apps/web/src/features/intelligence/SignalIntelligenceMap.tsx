    ...
    return (
      <div style={{ padding: 48, display: "flex", flexDirection: "column", gap: 32 }}>
        <section style={ds.card}>
          ...
          <p className="small-text" style={{ marginTop: 8 }}>Load an ontology bundle to explore signal patterns and relationships.</p>
          <p className="small-text" style={{ textAlign: "center", padding: 32, color: "var(--color-text-muted)" }}>No ontology data available.</p>
        </section>
      </div>
    );
  }
    ...
    <div style={{ padding: 48, display: "flex", flexDirection: "column", gap: 32 }}>
      ...
      <section style={ds.card}>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 20, flexWrap: "wrap" }}>
          {lensOptions.map((l) => (
            <button key={l.id} onClick={() => setLens(l.id)} style={pillStyle(lens === l.id)}>
              {l.label}
            </button>
          ))}
        </div>
        ...
        {loading ? (
          <p className="small-text" style={{ textAlign: "center", padding: 32, color: "var(--color-text-muted)" }}>Loading intelligence data...</p>
        ) : null}
      </section>
        ...
        <section style={ds.card}>
              ...
              {signalConcentration.slice(0, 20).map((item) => (
                ...
                >
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <span style={{ fontSize: 15, fontWeight: 600, color: "var(--color-text-primary)" }}>{item.name}</span>
                    <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                      <span style={{
                        padding: "2px 10px", borderRadius: 10, fontSize: 11, fontWeight: 600,
                        color: "var(--color-surface)", background: frequencyColor(item.frequency),
                      }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 10 }}>
                    <div style={{ flex: 1, height: 4, borderRadius: 2, background: "var(--color-surface-subtle)" }}>
                      <div style={{
          ...
          {venuePulses.length > 1 ? (
            <div style={{ marginTop: 28 }}>
              <p className="eyebrow" style={{ color: ds.accent, marginBottom: 12 }}>Cross-venue signal density</p>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: 12 }}>
                {venuePulses.filter((p) => p.latest_signal_count > 0).sort((a, b) => b.latest_signal_count - a.latest_signal_count).map((pulse) => (
                  ...
                  >
                    <span style={{ fontSize: 15, fontWeight: 600, color: "var(--color-text-primary)", display: "block", marginBottom: 10 }}>
                      {pulse.venue_name}
                    </span>
                    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13 }}>
                        <span style={{ color: "var(--color-text-muted)", fontWeight: 500 }}>Signals</span>
                        <span style={{ color: "var(--color-text-primary)" }}>{pulse.latest_signal_count}</span>
                      </div>
                      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13 }}>
                        <span style={{ color: "var(--color-text-muted)", fontWeight: 500 }}>Tasks</span>
                        <span style={{ color: "var(--color-text-primary)" }}>{pulse.latest_plan_task_count}</span>
                      </div>
                      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13 }}>
                        <span style={{ color: "var(--color-text-muted)", fontWeight: 500 }}>Load</span>
                        <span style={{ color: "var(--color-text-primary)" }}>{pulse.plan_load_classification ?? "unknown"}</span>
                      </div>
                    </div>
                  </div>
                ))}
      ...
      {lens === "domains" && !loading && bundle && (
        <section style={ds.card}>
          ...
          <p className="small-text" style={{ marginTop: 4 }}>How signals and failure modes are distributed across operational domains.</p>

          <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 20 }}>
            {domainDistribution.map((item) => (
              <div key={item.domain} className="ui-card" style={{ padding: "14px 20px" }}>
                <span style={{ fontSize: 15, fontWeight: 600, color: "var(--color-text-primary)", textTransform: "capitalize", display: "block", marginBottom: 6 }}>
                  {item.domain.replace(/_/g, " ")}
                </span>
                <div style={{ display: "flex", gap: 20, fontSize: 13, color: "var(--color-text-muted)", marginBottom: 10 }}>
                  <span>{item.signalCount} signals</span>
                  <span>{item.fmCount} failure modes</span>
                </div>
                <div style={{ height: 4, borderRadius: 2, background: "var(--color-surface-subtle)" }}>
                  <div style={{
      ...
      {lens === "chain" && !loading && bundle && (
        <section style={ds.card}>
          ...
          <p className="small-text" style={{ marginTop: 4 }}>Trace from signal through failure modes, response patterns, and intervention blocks.</p>

          {/* Signal selector */}
          <div style={{ marginTop: 16, marginBottom: 20, maxWidth: 420 }}>
            <Select
              options={bundle.signals.map((s) => ({ value: s.id, label: `${s.name} (${s.domain})` }))}
          ...
          {selectedChain ? (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {/* Signal node */}
              <div style={{ padding: "16px 20px", borderRadius: "var(--radius-md)", background: "rgba(108,92,231,0.06)", border: "1px solid rgba(108,92,231,0.15)" }}>
                <p className="eyebrow" style={{ color: ds.accent, marginBottom: 4 }}>SIGNAL</p>
                <span style={{ fontSize: 15, fontWeight: 600, color: "var(--color-text-primary)", display: "block" }}>{selectedChain.signal.name}</span>
                <p className="small-text" style={{ marginTop: 4 }}>{selectedChain.signal.description}</p>
                <div style={{ display: "flex", gap: 16, fontSize: 13, color: "var(--color-text-muted)", marginTop: 8 }}>
              ...
              {selectedChain.failureModes.map((fm) => (
                <div key={fm.id}>
                  <div style={{ padding: "14px 20px", borderRadius: "var(--radius-md)", background: "var(--color-surface)", borderLeft: `3px solid ${ds.danger}`, border: "1px solid var(--color-border-subtle)", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
                    <p className="eyebrow" style={{ color: ds.danger, marginBottom: 4 }}>FAILURE MODE (weight: {fm.weight})</p>
                    <span style={{ fontSize: 15, fontWeight: 600, color: "var(--color-text-primary)" }}>{fm.name}</span>
                  </div>

                  {/* Response patterns */}
                  {fm.responsePatterns.map((rp) => (
                    <div key={rp.id} style={{ marginLeft: 20, marginTop: 10 }}>
                      <div style={{ padding: "14px 20px", borderRadius: "var(--radius-md)", background: "var(--color-surface)", borderLeft: `3px solid ${ds.warning}`, border: "1px solid var(--color-border-subtle)", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
                        <p className="eyebrow" style={{ color: ds.warning, marginBottom: 4 }}>RESPONSE PATTERN (weight: {rp.weight})</p>
                        <span style={{ fontSize: 15, fontWeight: 600, color: "var(--color-text-primary)" }}>{rp.name}</span>
                      </div>

                      {/* Blocks */}
                      {rp.blocks.map((block: any) => (
                        <div key={block.id} style={{ marginLeft: 20, marginTop: 10 }}>
                          <div style={{ padding: "14px 20px", borderRadius: "var(--radius-md)", background: "var(--color-surface)", borderLeft: `3px solid ${ds.success}`, border: "1px solid var(--color-border-subtle)", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
                            <p className="eyebrow" style={{ color: ds.success, marginBottom: 4 }}>INTERVENTION BLOCK</p>
                            <span style={{ fontSize: 15, fontWeight: 600, color: "var(--color-text-primary)", display: "block", marginBottom: 6 }}>{block.name}</span>
              ...
              {selectedChain.failureModes.length === 0 && (
                <p className="small-text" style={{ textAlign: "center", padding: 24, color: "var(--color-text-muted)" }}>
      ...
      {lens === "timeline" && !loading && (
        <section style={ds.card}>
          ...
          <p className="small-text" style={{ marginTop: 4 }}>How the venue's signal landscape and execution load changed over time.</p>
              ...
              {timelineData.map((entry) => (
                <div key={entry.id} className="ui-card" style={{ padding: "14px 20px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                    ...
                    <span style={{ fontSize: 13, color: "var(--color-text-muted)" }}>{entry.load ?? "saved only"}</span>
                  </div>
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13 }}>
                      <span style={{ color: "var(--color-text-muted)", fontWeight: 500 }}>Signals</span>
                      <span style={{ color: "var(--color-text-primary)" }}>{entry.signalCount}</span>
                    </div>
                    <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13 }}>
                      <span style={{ color: "var(--color-text-muted)", fontWeight: 500 }}>Plan tasks</span>
                      <span style={{ color: "var(--color-text-primary)" }}>{entry.taskCount}</span>
                    </div>
                    {entry.ontology && (
                      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13 }}>
                        <span style={{ color: "var(--color-text-muted)", fontWeight: 500 }}>Ontology</span>
                        <span style={{ color: "var(--color-text-muted)" }}>{entry.ontology}</span>
                      </div>
                    )}
                  </div>
                  {entry.topSignals.length > 0 && (
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 10 }}>
                      {entry.topSignals.map((name) => (
                        <span key={name} style={{ padding: "2px 10px", borderRadius: 10, background: "var(--color-surface-subtle)", fontSize: 11, color: "var(--color-text-muted)", fontWeight: 500 }}>
                          {name}
                        </span>
                      ))}
                    </div>
      ...
      {lens === "heatmap" && (
        <section style={ds.card}>
          ...
          <p className="small-text" style={{ marginTop: 4 }}>Cross-tabular view: rows are signals sorted by frequency, columns are venues. Cell intensity shows occurrence rate.</p>
